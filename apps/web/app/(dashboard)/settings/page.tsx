    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Section title="Profil">
        <div className="flex items-center gap-4 mb-5">
          <div className="w-14 h-14 rounded-full flex items-center justify-center text-white font-display font-bold text-xl shadow-lg flex-shrink-0"
            style={{ background: 'linear-gradient(135deg, #1A56DB, #2B7FD4)' }}>
            Z
          </div>
          <div>
            <h2 className="font-display font-bold text-[#141110] text-base leading-none">Zels</h2>
            <p className="text-[#A8A29E] text-sm mt-1">mvdgroupindonesia@gmail.com</p>
            <span className="inline-block mt-1.5 px-2 py-0.5 rounded-full bg-[#EBF0FF] text-[#1A56DB] text-[11px] font-semibold">
              Paket Free
            </span>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <button className="flex items-center justify-center gap-2 bg-white border-2 border-[#E5E2DD] text-[#141110]
            font-semibold rounded-xl px-4 py-2.5 text-sm hover:border-[#1A56DB]/30 transition-all">
            <IcoEdit /> Edit Profil
          </button>
          <Link href="/#pricing"
            className="flex items-center justify-center gap-2 text-white font-semibold rounded-xl px-4 py-2.5 text-sm text-center
              hover:opacity-90 hover:shadow-md hover:shadow-[#1A56DB]/20 transition-all"
            style={{ background: 'linear-gradient(135deg, #1A56DB, #2B7FD4)' }}>
            Upgrade Paket
          </Link>
        </div>
      ...
      <Section title="Storage">
        <div className="flex justify-between items-baseline mb-2">
          <span className="text-[#141110] font-semibold text-sm">3,5 GB</span>
          <span className="text-[#A8A29E] text-xs">dari 15 GB</span>
        </div>
        <div className="h-2.5 bg-[#F2F0ED] rounded-full overflow-hidden mb-3">
          <div className="h-full rounded-full" style={{ width: '23%', background: 'linear-gradient(90deg, #1A56DB, #60A5FA)' }} />
        </div>
          ...
          {[
            ...
            <div key={label} className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-[#F2F0ED] text-[#6B6560]">
                  <Icon />
                </div>
      ...
      <Section title="Akun">
        <div className="space-y-1.5">
          {[
            { label: 'Ganti Password',   Icon: IcoKey,   accent: '#1A56DB', bg: '#EBF0FF' },
            { label: 'Kelola Perangkat', Icon: IcoPhone,  accent: '#059669', bg: '#ECFDF5' },
            { label: 'Referral & Bonus', Icon: IcoGift,   accent: '#7C3AED', bg: '#F5F3FF' },
            { label: 'Pusat Bantuan',    Icon: IcoHelp,   accent: '#D97706', bg: '#FFF7ED' },
          ].map(({ label, Icon, accent, bg }) => (
            <button key={label}
              className="w-full flex items-center gap-3.5 p-3.5 rounded-xl
                hover:bg-[#FAFAF8] hover:border-[#E5E2DD] border border-transparent transition-all duration-150 text-left group">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                style={{ background: bg, color: accent }}>
                <Icon />
              </div>
              <span className="font-medium text-[#141110] text-sm flex-1">{label}</span>
              <span className="text-[#D4CFC9] group-hover:text-[#A8A29E] transition-colors">
                <IcoChevronR />
              </span>
            </button>
          ))}
      ...
      <div className="bg-[#FFF1F2] border border-red-100 rounded-2xl p-5">
        ...
        <button className="w-full flex items-center justify-center gap-2 text-red-500 font-semibold text-sm
          py-3 rounded-xl bg-white border border-red-200 hover:bg-red-50 hover:border-red-300 transition-all">
          <IcoTrash /> Hapus Akun
        </button>
      </div>
